...
def draw_control_polygon(key_points):
    ...
    for i in range(0, segment_number):
        keys_on_one_polygon = key_points[4*i*3:4*(i+1)*3]
...
def interpolate_bezier_position_at(t, p0, p1, p2, p3):
    b0 = pow(1 - t, 3)
    b1 = 3*t * pow(1 - t, 2)
    b2 = 3*pow(t,2)*(1-t)
    b3 = pow(t,3)
...
def gen_bezier_curve_points(key_points):
    key_number = len(key_points)
    if key_number < 4:
        return
    step_size = 0.01
    step_count = int(1/step_size)
    segment_number = int((key_number ) / 4)
    ...
    for i in range(0, segment_number):
        ...
        for s in range(0, step_count+1):
            t = s * step_size
            one_position_on_curve = interpolate_bezier_position_at(t, p0, p1, p2, p3)
...
while (
    ...
    if len(key_points) > 0:
        ...
        positions_on_curve = gen_bezier_curve_points(Point3.num_list_to_point_list(key_points))
