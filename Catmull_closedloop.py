...
def catmull_rom_position_at(t, p0, p1, p2, p3):
    b0 = 0.5*(pow(-t, 3)+2*pow(t,2)-t)
...
def gen_catmull_rom_points(key_points):
    key_number = len(key_points)
    if key_number < 4:
        return
    
    extra_key_points = key_points[-2:] + key_points + key_points[:2]
    key_number += 4  
    ...
    for i in range(1, key_number - 2):
        # Wrap around for the closed loop
        p0 = extra_key_points[i - 1]
        p1 = extra_key_points[i]
        p2 = extra_key_points[i + 1]
        p3 = extra_key_points[i + 2]

        for s in range(step_count + 1):
            t = s * step_size
            one_position_on_curve = catmull_rom_position_at(t, p0, p1, p2, p3)
...
while (
    ...
    if len(key_points) > 0:
        ...
        positions_on_curve = gen_catmull_rom_points(Point3.num_list_to_point_list(key_points))
